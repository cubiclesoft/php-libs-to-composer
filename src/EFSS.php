...
?><?php
	...
	class EFSS
	{
		...
		{
			if (file_exists($basefile) || file_exists($basefile . ".updates") || file_exists($basefile . ".serial"))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Encrypted file storage system already exists."), "errorcode" => "already_exists");
			if ($blocksize == 0 || $blocksize % 4096 != 0 || $blocksize > 32768)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The block size must be a multiple of 4096 and able to fit into an 'unsigned short'."), "errorcode" => "invalid_blocksize");
			if (($dirmode & EFSS_DIRMODE_COMPRESS) && (!function_exists("gzcompress") || !function_exists("gzuncompress")))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The compressed directory mode is unsupported because one or more required PHP functions are unavailable."), "errorcode" => "unsupported_dirmode");
			...
			$this->defownername = "";
			$this->defgroupname = "";
			if (!$this->Lock($lockfile, EFSS_MODE_EXCL))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to obtain exclusive lock."), "errorcode" => "lock_failed");
			$this->fp = fopen($basefile, "wb");
			if ($this->fp === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open block file."), "errorcode" => "block_file_open");
			$this->fp2 = fopen($basefile . ".updates", "wb");
			if ($this->fp2 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open updates file."), "errorcode" => "updates_file_open");
			$this->fp3 = fopen($basefile . ".hashes", "wb");
			if ($this->fp3 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open hashes file."), "errorcode" => "hashes_file_open");
			...
			if (!$result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to write the first block."), "errorcode" => "first_block_write", "info" => $result);
			...
			if (!$result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to write the root unused block."), "errorcode" => "root_unused_block_write", "info" => $result);
			...
			if (!$result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to write the root directory block."), "errorcode" => "root_dir_block_write", "info" => $result);
		...
		{
			if (!file_exists($basefile) || !file_exists($basefile . ".updates") || !file_exists($basefile . ".serial") || file_exists($basefile . ".blocknums") || file_exists($basefile . ".partial"))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Encrypted file storage system does not exist."), "errorcode" => "does_not_exist");
			if ($blocksize == 0 || $blocksize % 4096 != 0 || $blocksize > 32768)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The block size must be a multiple of 4096 and able to fit into an 'unsigned short'."), "errorcode" => "invalid_blocksize");
			if (count($incrementals) && $mode != EFSS_MODE_READ)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The encrypted file storage system can only be opened for reading when using the incrementals option."), "errorcode" => "reading_only");
			if (count($incrementals) && $reversediff)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The encrypted file storage system can't create a reverse diff when using the incrementals option."), "errorcode" => "reverse_diff_error");
			if ($reversediff !== false && $mode == EFSS_MODE_READ)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The encrypted file storage system can't create a reverse diff when opening for reading."), "errorcode" => "writing_only");
			if (file_exists($basefile . ".readonly") && $mode != EFSS_MODE_READ)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The encrypted file storage system can only be opened for reading."), "errorcode" => "reading_only");
			...
			$this->defownername = "";
			$this->defgroupname = "";
			if (!$this->Lock($lockfile, $mode, $waitforlock))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to obtain lock."), "errorcode" => "lock_failed");
			...
			if ($mode == EFSS_MODE_EXCL)
			{
				...
				$this->lastwrite = \CubicleSoft\EFSS::ConvertFromUTCDateTime(fread($this->fp2, 20));
				$this->written = false;
				fseek($this->fp2, 0);
			...
			if (count($incrementals))
			{
				...
				{
					if (!file_exists($filename) || !file_exists($filename . ".updates") || !file_exists($filename . ".serial") || !file_exists($filename . ".blocknums") || file_exists($filename . ".partial"))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Encrypted file storage system incremental '%s' does not exist.", $filename), "errorcode" => "does_not_exist");

					$serial2 = trim(file_get_contents($filename . ".serial"));
					if ($serial !== $serial2)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Incremental block file '%s' serial does not match base file serial.", $filename), "errorcode" => "increment_serial_mismatch");

					$fp = fopen($filename, "rb");
					if ($fp === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open incremental block file '%s'.", $filename), "errorcode" => "increment_block_file_open");

					$fp2 = fopen($filename . ".blocknums", "rb");
					if ($fp2 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open incremental block numbers file '%s'.", $filename), "errorcode" => "increment_block_nums_file_open");
			...
			if (!$result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The first block of the encrypted file storage system is corrupt."), "errorcode" => "first_block_corrupt", "info" => $result);

			// Finalize initialization with information from the first block.
			$this->firstblock = new EFSS_FirstBlock;
			if (!$this->firstblock->unserialize($result["data"]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The first block of the encrypted file storage system is invalid."), "errorcode" => "first_block_failure");
			if ($this->blocksize != $this->firstblock->blocksize)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The first block of the encrypted file storage system is invalid.  Block sizes do not match."), "errorcode" => "first_block_failure");
			$this->dirmode = $this->firstblock->dirmode;
			$this->timestamp = $this->firstblock->timestamp;
			if (($this->dirmode & EFSS_DIRMODE_COMPRESS) && (!function_exists("gzcompress") || !function_exists("gzuncompress")))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The compressed directory mode is unsupported because one or more required PHP functions are unavailable."), "errorcode" => "unsupported_dirmode");
			...
			if ($this->rdiff)
			{
				...
				if ($this->rdiff_fp === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open reverse diff block file '%s'.", $basefile . ".rdiff"), "errorcode" => "rdiff_block_file_open");

				$this->rdiff_fp2 = fopen($basefile . ".rdiff.updates", "ab");
				if ($this->rdiff_fp2 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open reverse diff updates file '%s'.", $basefile . ".rdiff.updates"), "errorcode" => "rdiff_updates_file_open");

				$this->rdiff_fp3 = fopen($basefile . ".rdiff.hashes", "ab");
				if ($this->rdiff_fp3 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open reverse diff hashes file '%s'.", $basefile . ".rdiff.hashes"), "errorcode" => "rdiff_hashes_file_open");

				$this->rdiff_fp4 = fopen($basefile . ".rdiff.blockmap", "r+b");
				if ($this->rdiff_fp4 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open reverse diff block mapping file '%s'.", $basefile . ".rdiff.blockmap"), "errorcode" => "rdiff_block_map_file_open");
				...
				if ($this->rdiff_maxblocks > $this->firstblock->nextblock)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Reverse diff block info file '%s' is corrupt.", $basefile . ".rdiff.blockinfo"), "errorcode" => "rdiff_block_info_corrupt");
			}

			// Read the last unused block that has available blocks.
			$result = $this->ReloadUnused();
			if (!$result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("An error occurred while loading available unused blocks."), "errorcode" => "unused_reload", "info" => $result);
			...
			if (!$result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The root directory block of the encrypted file storage system is corrupt or invalid."), "errorcode" => "root_dir_block_corrupt", "info" => $result);
		...
		{
			if (!$this->mounted)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not mounted."), "errorcode" => "no_mount");
		...
		{
			if (!$this->mounted)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not mounted."), "errorcode" => "no_mount");
			if ($this->mode !== EFSS_MODE_EXCL)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not mounted with exclusive lock."), "errorcode" => "read_only_lock");

			// Load the path information.
			$result = $this->LoadPath($pathname, false, EFSS_DIRTYPE_DIR, true);
			if ($result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("'%s' already exists.", $pathname), "errorcode" => "already_exists");
		...
		public function rmdir($pathname, $recursive = false)
		{
			...
			if (!$result["success"])  return $result;
			if (!count($result["dirinfo"]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Root directory can't be removed."), "errorcode" => "root_dir_locked");
			...
			{
				if ($result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Directory not empty."), "errorcode" => "dir_not_empty");
				if ($result["errorcode"] != "dir_end")  return $result;
			}
			...
			if ($result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Should never happen."), "errorcode" => "impossible");
		...
		public function opendir($path)
		{
			...
			if (!$result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Failed to read directory block %u.", $blocknum), "errorcode" => "dir_block_read", "info" => $result);
		...
		{
			if (!is_array($handle))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not a valid handle."), "errorcode" => "invalid_handle");
			if (!$this->mounted)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not mounted."), "errorcode" => "no_mount");
			if ($handle["done"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Reached end of directory entries."), "errorcode" => "dir_end");
			...
			else
			{
				...
				if (!count($handle["currdir"]->entries))
				{
					...
					return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Reached end of directory entries."), "errorcode" => "dir_end");
				}
		...
		public function chown($dirfile, $ownername)
		{
			...
			if (!$result["success"])  return $result;
			if (!count($result["dirinfo"]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Root directory can't be changed."), "errorcode" => "root_dir_locked");
		...
		public function copy($source, $dest, $copymode = EFSS_COPYMODE_DEFAULT)
		{
			...
			else
			{
				// Read in 1MB.
				$data = $sfp->Read(1048576);
				if ($data === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("An error occurred while reading '%s'.", $source), "errorcode" => "source_read_error");
				...
				if (strlen($data) < 1048576)
				{
					...
					{
						if (!file_put_contents($dest, $data))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Error writing to '%s'.", $dest), "errorcode" => "write_error");
					}
					else
					{
				...
				else
				{
					...
					while (strlen($data) > 0)
					{
						if ($dfp->Write($data) === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("An error occurred while writing '%s'.", $dest), "errorcode" => "dest_write_error");
		...
		{
			if ($mode !== "rb" && $mode !== "wb")  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Invalid fopen() mode.  Only 'rb' and 'wb' are supported."), "errorcode" => "fopen_mode");
			...
			if (!$result["success"])  return $result;
			if (!count($result["dirinfo"]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Root directory can't be opened as a file."), "errorcode" => "root_dir_locked");
			$filename = $result["path"];
			if (isset($this->openfiles[$filename]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("File is already open for reading or writing."), "errorcode" => "already_open");
			$fileinfo = $result["dirinfo"][count($result["dirinfo"]) - 1][4];
			if ($fileinfo->type != EFSS_DIRTYPE_FILE)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("'%s' is not a file.", $filename), "errorcode" => "not_a_file");

			// Generate a read only file handle.
			if (($fileinfo->permflags & EFSS_FLAG_COMPRESS) && !\CubicleSoft\DeflateStream::IsSupported())  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The file cannot be opened because one or more required PHP functions are unavailable."), "errorcode" => "unsupported_filemode");
			$fp = array(
				"write" => false,
				"eof" => false,
				"pos" => 0,
			...
			{
				$fp["data"] = ($fileinfo->permflags & EFSS_FLAG_COMPRESS ? \CubicleSoft\DeflateStream::Uncompress($fileinfo->data) : $fileinfo->data);
				$fp["origdata"] = $fp["data"];
				$fp["blocknum"] = 0;
			}
			else
		...
		public function fopen_write($filename, $filemode = 0664, $compress = EFSS_COMPRESS_DEFAULT, $ownername = false, $groupname = false, $created = false, $data = false)
		{
			...
			if ($compress == EFSS_COMPRESS_DEFAULT && !\CubicleSoft\DeflateStream::IsSupported())  $compress = EFSS_COMPRESS_NONE;
			...
			if ($result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("'%s' already exists.", $filename), "errorcode" => "already_exists");
			if ($result["errorcode"] != "path_not_found")  return $result;
			...
			if ($data !== false)
			{
				...
				{
					$data2 = \CubicleSoft\DeflateStream::Compress($data);
		...
		{
			if (!$this->mounted)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not mounted."), "errorcode" => "no_mount");
			if (!isset($this->openfiles[$fp]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Invalid file handle."), "errorcode" => "invalid_handle");
		...
		public function fflush($fp)
		{
			if (!$this->mounted)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not mounted."), "errorcode" => "no_mount");
			if (!isset($this->openfiles[$fp]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Invalid file handle."), "errorcode" => "invalid_handle");
			if (!$this->openfiles[$fp]["write"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("File is open for reading only."), "errorcode" => "read_only");
		...
		private function internal_fflush($fp, $finalize = false)
		{
			...
			if ($finalize)
			{
				...
				if ($this->openfiles[$fp]["updatedir"])
				{
					...
					if ($result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("File not found in directory block."), "errorcode" => "file_not_found");
					if ($result["errorcode"] != "already_exists")  return $result;
		...
		public function fgetcsv($fp, $delimiter = ",", $enclosure = "\"")
		{
			if (!$this->mounted)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not mounted."), "errorcode" => "no_mount");
			if (!isset($this->openfiles[$fp]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Invalid file handle."), "errorcode" => "invalid_handle");
			if ($this->openfiles[$fp]["write"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("File is open for writing only."), "errorcode" => "write_only");
			if ($this->openfiles[$fp]["eof"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("End of file reached."), "errorcode" => "eof");
		...
		public function fread($fp, $len)
		{
			...
			if ($this->openfiles[$fp]["write"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("File is open for writing only."), "errorcode" => "write_only");
			if ($this->openfiles[$fp]["eof"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("End of file reached."), "errorcode" => "eof");
			if ($len < 0)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Invalid length."), "errorcode" => "invalid_length");
		...
		{
			if ($this->openfiles[$fp]["blocknum"] == 0 && !isset($this->openfiles[$fp]["inflate"]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("End of file reached."), "errorcode" => "eof");
			...
			else
			{
				...
				if (!$fileinfo->unserialize($result["data"]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("File block %u of the encrypted file storage system is invalid.", $blocknum), "errorcode" => "file_block_failure");
		...
		public function fseek($fp, $offset, $whence = SEEK_SET)
		{
			...
			if ($whence != SEEK_SET)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Invalid whence."), "errorcode" => "invlaid_whence");

			$this->openfiles[$fp]["eof"] = false;

			if ($offset < 0)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Invalid offset."), "errorcode" => "invalid_offset");
		...
		public function readlink($path)
		{
			...
			if (isset($this->openfiles[$path]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not a symbolic link."), "errorcode" => "not_symlink");

			$result = $this->LoadPath($path, false, EFSS_DIRTYPE_ANY, true, true);
			if (!$result["success"])  return $result;
			if (!count($result["dirinfo"]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not a symbolic link."), "errorcode" => "not_symlink");
			$fileinfo = $result["dirinfo"][count($result["dirinfo"]) - 1][4];

			if ($fileinfo->type != EFSS_DIRTYPE_SYMLINK)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not a symbolic link."), "errorcode" => "not_symlink");
		...
		public function rename($oldpath, $destpath)
		{
			...
			if (!$result["success"])  return $result;
			if (!count($result["dirinfo"]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Root directory can't be renamed."), "errorcode" => "root_dir_locked");
			$filename = $result["dirinfo"][count($result["dirinfo"]) - 1][0];
			$blocknum = $result["dirinfo"][count($result["dirinfo"]) - 1][3];
			...
			if ($result["success"])
			{
				...
				return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("'%s' already exists.", $destpath), "errorcode" => "already_exists");
			}
		...
		public function symlink($target, $link, $ownername = false, $groupname = false, $created = false)
		{
			...
			if ($result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("'%s' already exists.", $link), "errorcode" => "already_exists");
			if ($result["errorcode"] != "path_not_found")  return $result;
			if ($result["dirinfopos"] < count($result["dirinfo"]) - 1)  return $result;
			$blocknum = $result["blocknum"];
		...
		public function unlink($filename)
		{
			...
			$result = $this->LoadPath($filename, false, EFSS_DIRTYPE_ANY, true, true);
			if (!$result["success"])  return $result;
			if (!count($result["dirinfo"]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to unlink the root directory."), "errorcode" => "root_dir_locked");
			$filename = $result["dirinfo"][count($result["dirinfo"]) - 1][0];
			$blocknum = $result["dirinfo"][count($result["dirinfo"]) - 1][3];
			...
			$entry = $result["dir"]->entries[$result["pos"]];
			$type = $entry->type;
			if ($type == EFSS_DIRTYPE_DIR)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to unlink a directory."), "errorcode" => "directory_unlink");
			if ($type != EFSS_DIRTYPE_FILE && $type != EFSS_DIRTYPE_SYMLINK)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unknown directory entry type."), "errorcode" => "impossible");
		...
		public function CheckFS($blockcallbackfunc = false, $filecallbackfunc = false)
		{
			...
			for ($x = 0; $x < $numblocks; $x++)
			{
				...
				{
					if ($x > 0)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Invalid 'firstblock' found at block %u.", $x), "errorcode" => "invalid_firstblock");
				}
				else if ($result["type"] === EFSS_BLOCKTYPE_DIR || $result["type"] === EFSS_BLOCKTYPE_FILE || $result["type"] === EFSS_BLOCKTYPE_UNUSED_LIST)
				{
					if (strlen($result["data"]) < 4)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Block %u of the encrypted file storage system is invalid.", $x), "errorcode" => "block_failure");

					// Save a few cycles by only processing 4 bytes of data.
					$x2 = \CubicleSoft\EFSS::UnpackInt(substr($result["data"], 0, 4));
					if ($x2 > 0)
					{
						if ($x2 > $numblocks || $x2 < 3 || $x2 == $x)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Block %u references an out of range or invalid block %u.", $x, $x2), "errorcode" => "block_out_of_range");

						$byte = ord($referenced[(int)($x2 / 8)]);
						$pos = $x2 % 8;
						if ($byte & $mask[$pos])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Block %u references an already referenced block %u.", $x, $x2), "errorcode" => "block_already_referenced");
					...
					if ($result["type"] === EFSS_BLOCKTYPE_DIR)
					{
						...
						foreach ($result["dir"]->entries as $entry)
						{
							...
							{
								if ($entry->data > $numblocks || $entry->data < 3 || $entry->data == $x)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Directory block %u references an out of range or invalid block %u with name '%s'.", $x, $entry->data, $entry->name), "errorcode" => "block_out_of_range");

								$byte = ord($referenced[(int)($entry->data / 8)]);
								$pos = $entry->data % 8;
								if ($byte & $mask[$pos])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Directory block %u references an already referenced block %u with name '%s'.", $x, $entry->data, $entry->name), "errorcode" => "block_already_referenced");
							...
							else if ($entry->type == EFSS_DIRTYPE_FILE)
							{
								...
								{
									if ($entry->data > $numblocks || $entry->data < 3 || $entry->data == $x)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("File block %u references an out of range or invalid block %u with name '%s'.", $x, $entry->data, $entry->name), "errorcode" => "block_out_of_range");

									$byte = ord($referenced[(int)($entry->data / 8)]);
									$pos = $entry->data % 8;
									if ($byte & $mask[$pos])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("File block %u references an already referenced block %u with name '%s'.", $x, $entry->data, $entry->name), "errorcode" => "block_already_referenced");
					...
					else if ($result["type"] === EFSS_BLOCKTYPE_UNUSED_LIST)
					{
						...
						if (!$unusedblock->unserialize($result["data"]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unused block %u of the encrypted file storage system is invalid.", $x), "errorcode" => "unused_block_failure");
				...
				{
					return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Block %u is an invalid type.", $x), "errorcode" => "block_type_invalid");
				}
			...
			for ($x = 0; $x < $y; $x++)
			{
				if (ord($referenced[$x]) !== 0xFF)
				{
					...
					for ($x2 = 0; $x2 < 8; $x2++)
					{
						if (($byte & $mask[$x2]) == 0)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Block %u is never referenced.", $x + $x2), "errorcode" => "block_not_referenced");
					}
				}
			}
				...
				else
				{
					...
					else if ($result["info"]->type == EFSS_DIRTYPE_FILE && !($result["info"]->permflags & EFSS_FLAG_INLINE))
					{
						...
						while ($blocknum > 0)
						{
							...
							if (!$result2["success"])  return $result2;
							if (strlen($result2["data"]) < 4)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Block %u of the encrypted file storage system is invalid.", $blocknum), "errorcode" => "block_failure");

							// Save a few cycles by only processing 4 bytes of data.
							$blocknum = \CubicleSoft\EFSS::UnpackInt(substr($result2["data"], 0, 4));
				...
				for ($x = 0; $x < $y; $x++)
				{
					if (is_string($dirinfo[$x]))
					{
						...
						do
						{
							...
							if (!$result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Failed to read directory block %u.", $num), "errorcode" => "dir_block_read", "info" => $result);
						...
						if (!count($result["dir"]->entries))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Path not found (%s).", $dirinfo[$x]), "errorcode" => "path_not_found", "dirinfo" => $dirinfo, "dirinfopos" => $x, "blocknum" => $firstblock, "symlinks" => $symlinks);
						if ($result["dir"]->entries[0]->type == EFSS_DIRTYPE_SYMLINK && (!$lastsymlink || $x < $y - 1))
						{
							$path = $result["dir"]->entries[0]->data;
							if (substr($path, -1) == "/")  $path = substr($path, 0, -1);
							if (isset($symlinks[$path]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Symlink infinite loop detected."), "errorcode" => "infinite_loop", "path" => $path);
							$symlinks[$path] = true;
							$x++;
						...
						if ($x < $y - 1 && $result["dir"]->entries[0]->type != EFSS_DIRTYPE_DIR)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("%s is not a directory.", $dirinfo[$x]), "errorcode" => "not_directory");
						else if ($x == $y - 1 && $lastentrytype != EFSS_DIRTYPE_ANY && $result["dir"]->entries[0]->type != $lastentrytype)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("%s is not the required type.", $dirinfo[$x]), "errorcode" => "incorrect_type");
		...
		private function ReadDirBlock($blocknum, $findname = false, $allnames = false)
		{
			if (!isset($this->dirblockcache[$blocknum]))
			{
				...
				if (!$result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Directory block %u of the encrypted file storage system is corrupt.", $blocknum), "errorcode" => "dir_block_corrupt", "info" => $result);
			...
			if (!$dir->unserialize($this->dirblockcache[$blocknum], $this->timestamp, $findname, (($this->dirmode & EFSS_DIRMODE_CASE_INSENSITIVE) > 0), $allnames))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Directory block %u of the encrypted file storage system is invalid.", $blocknum), "errorcode" => "dir_block_failure");
		...
		private function FindDirInsertPos($firstblock, $newname)
		{
			...
			do
			{
				...
				if ($this->dirinsertmapcache[$blocknum][0] !== false)
				{
					...
					if ($cmp == 0)
					{
						...
						return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("'%s' already exists.", $newname), "errorcode" => "already_exists", "firstblock" => $firstblock, "blocknum" => $blocknum, "dir" => $dir, "pos" => 0);
					}
			...
			if ($this->dirmode & EFSS_DIRMODE_CASE_INSENSITIVE)
			{
				...
				if ($x < $y && strnatcasecmp($dir->entries[$x]->name, $newname) == 0)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("'%s' already exists.", $newname), "errorcode" => "already_exists", "firstblock" => $firstblock, "blocknum" => $blocknum, "dir" => $dir, "pos" => $x);
			}
			else
			{
				for ($x = 0; $x < $y && strnatcmp($dir->entries[$x]->name, $newname) < 0; $x++);
				if ($x < $y && strnatcmp($dir->entries[$x]->name, $newname) == 0)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("'%s' already exists.", $newname), "errorcode" => "already_exists", "firstblock" => $firstblock, "blocknum" => $blocknum, "dir" => $dir, "pos" => $x);
			}
		...
		private function WriteDirBlock($dir, $blocknum, $firstblock)
		{
			...
			do
			{
				...
				$data = $dir->serialize();
				if ($data === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Serialization of directory block %u failed.", $blocknum), "errorcode" => "serialize_dir");
				...
				while (strlen($data) > $this->blocksize - 30)
				{
					...
					if (!count($dir->entries))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Directory entry '%s' is larger than the system block size.", $prepend[0]->name), "errorcode" => "data_size");
		...
		private function ReloadUnused()
		{
			...
			if (!$result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The first unused block of the encrypted file storage system is corrupt."), "errorcode" => "unused_block_corrupt", "info" => $result);

			// Load the data.
			$this->unusedblock = new EFSS_Unused;
			if (!$this->unusedblock->unserialize($result["data"]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The first unused block of the encrypted file storage system is invalid."), "errorcode" => "unused_block_failure");
			...
			while ($this->unusedblock->nextblock > 0)
			{
				...
				if (!$result["success"])  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("An unused block of the encrypted file storage system is corrupt."), "errorcode" => "unused_block_corrupt", "info" => $result);

				$unusedblock = new EFSS_Unused;
				if (!$unusedblock->unserialize($result["data"]))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("An unused block of the encrypted file storage system is invalid."), "errorcode" => "unused_block_failure");
		...
		private function FreeLinkedList($blocknum)
		{
			...
			while ($blocknum > 1)
			{
				...
				if (!$result["success"])  return $result;
				if ($result["type"] !== EFSS_BLOCKTYPE_DIR && $result["type"] !== EFSS_BLOCKTYPE_FILE && $result["type"] !== EFSS_BLOCKTYPE_UNUSED_LIST)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Block %u of the encrypted file storage system is not a linked list.", $blocknum), "errorcode" => "block_is_not_list");
				if (strlen($result["data"]) < 4)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Block %u of the encrypted file storage system is invalid.", $blocknum), "errorcode" => "block_failure");

				// Save a few cycles by only processing 4 bytes of data.
				$blocknum2 = \CubicleSoft\EFSS::UnpackInt(substr($result["data"], 0, 4));
		...
		private function Lock($lockfile, $mode, $waitforlock = true)
		{
			...
			$this->mode = $mode;
			$this->readwritelock = new \CubicleSoft\ReadWriteLock($lockfile === false ? $this->basefile : $lockfile);
		...
		private function RawWriteBlock($data, $blocknum, $type)
		{
			...
			if ($blocknum < 0)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Invalid block number."), "errorcode" => "invalid_block_number");

			if (!$this->mounted)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not mounted."), "errorcode" => "no_mount");
			if ($this->mode !== EFSS_MODE_EXCL)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not mounted with exclusive lock."), "errorcode" => "read_only_lock");
			if (strlen($data) > $this->blocksize - 30)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Too much data.  Max size is %u.  Received %u.", $this->blocksize - 30, strlen($data)), "errorcode" => "data_size");
			if (strlen($type) != 1)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Invalid block type specified."), "errorcode" => "invalid_block_type");
			...
			if (strlen($block) > $this->blocksize)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Should never happen."), "errorcode" => "impossible");
			...
			{
				\CubicleSoft\EFSS::RawSeek($this->rdiff_fp4, $blocknum * 4);
			...
			$data = \CubicleSoft\EFSS::ConvertToUTCDateTime($this->ts);
		...
		private function RawReadBlock($blocknum, $type)
		{
			...
			for ($x = 0; $x < count($this->incrementals) && !$found; $x++)
			{
				...
				if (is_resource($this->incrementals[$x][1]))
				{
					...
					while ($min < $max)
					{
						...
						fseek($fp2, $mid * 4);
						$blocknum2 = \CubicleSoft\EFSS::UnpackInt(fread($fp2, 4));
					...
					{
						\CubicleSoft\EFSS::RawSeek($fp, $min * $this->blocksize);
						$found = true;
					}
				}
				else if (is_string($this->incrementals[$x][1]))
				{
					...
					if ($pos !== false)
					{
						\CubicleSoft\EFSS::RawSeek($fp, ($pos / 4) * $this->blocksize);
						$found = true;
					}
				}
			}
			...
			if (strlen($block) !== $this->blocksize)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Error reading data.  Possible corruption detected."), "errorcode" => "data_read");
			...
			if ($type2 === EFSS_BLOCKTYPE_ANY || ($type !== EFSS_BLOCKTYPE_ANY && $type !== $type2))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Invalid block type detected.  Possible corruption or decryption failure."), "errorcode" => "invalid_block_type_verify");
			$size = \CubicleSoft\EFSS::UnpackInt(substr($block, 8, 2));
			$data = (string)substr($block, 10, $size);
			if (strlen($data) !== $size)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Data size corruption detected.  Possible decryption failure."), "errorcode" => "data_size_verify");
			$hash = (string)substr($block, 10 + $size, 20);
			if (strlen($hash) !== 20)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Hash size corruption detected.  Possible decryption failure."), "errorcode" => "hash_size_verify");
			if ($hash !== sha1($data, true))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Hash verification failure.  Data corruption or decryption failure."), "errorcode" => "hash_verify");
		...
		{
			return \CubicleSoft\EFSS::RawSeek($this->fp, $blocknum * $this->blocksize);
		}

		private function RawSeekUpdate($blocknum)
		{
			return \CubicleSoft\EFSS::RawSeek($this->fp2, $blocknum * 20);
		}

		private function RawSeekHashes($blocknum)
		{
			return \CubicleSoft\EFSS::RawSeek($this->fp3, $blocknum * 36);
		}
	...
	class EFSS_FirstBlock
	{
		...
		public function unserialize($data)
		{
			...
			$this->version = \CubicleSoft\EFSS::UnpackInt(substr($data, 0, 2));
			if ($this->version > EFSS_VERSION)  return false;
			$this->blocksize = \CubicleSoft\EFSS::UnpackInt(substr($data, 2, 2));
			if ($this->blocksize % 4096 != 0)  return false;

			$this->dirmode = ord(substr($data, 4, 1));

			$this->nextblock = \CubicleSoft\EFSS::UnpackInt(substr($data, 5, 4));

			$this->timestamp = (substr($data, 9, 1) === "\x01" ? EFSS_TIMESTAMP_UNIX : EFSS_TIMESTAMP_UTC);
			if ($this->timestamp == EFSS_TIMESTAMP_UNIX)  $this->created = \CubicleSoft\EFSS::UnpackInt(substr($data, 10, 4));
			else
			{
				if (strlen($data) < 30)  return false;

				$this->created = \CubicleSoft\EFSS::ConvertFromUTCDateTime(substr($data, 10, 20));
			}
		...
		public function serialize()
		{
			...
			else  $data .= \CubicleSoft\EFSS::ConvertToUTCDateTime($this->created);
	...
	class EFSS_DirEntries
	{
		...
		public function unserialize($data, $timestamp, $findname = false, $insensitive = true, $allnames = false)
		{
			...
			$this->nextblock = \CubicleSoft\EFSS::UnpackInt(substr($data, 0, 4));
			$data = substr($data, 4);
			...
			while (strlen($data) > 1)
			{
				$datalen = \CubicleSoft\EFSS::UnpackInt(substr($data, 0, 2));
				...
				$namelen = \CubicleSoft\EFSS::UnpackInt(substr($data2, 1 + $timestampsize, 2));
				...
				if ($findname === false || ($insensitive ? strcasecmp($findname, $name) == 0 : $findname === $name))
				{
					...
					if ($this->timestamp == EFSS_TIMESTAMP_UNIX)  $created = \CubicleSoft\EFSS::UnpackInt(substr($data2, 1, 4));
					else  $created = \CubicleSoft\EFSS::ConvertFromUTCDateTime(substr($data2, 1, 20));
					$data2 = substr($data2, 1 + $timestampsize + 2 + $namelen);

					$entry = new EFSS_DirEntry_DirFile;

					if (strlen($data2) < 4)  return false;
					$entry->permflags = \CubicleSoft\EFSS::UnpackInt(substr($data2, 0, 2));

					$len = \CubicleSoft\EFSS::UnpackInt(substr($data2, 2, 2));
					...
					$len = \CubicleSoft\EFSS::UnpackInt(substr($data2, 0, 2));
					...
					switch ($type)
					{
						case EFSS_DIRTYPE_DIR:
						{
							...
							if (strlen($data2) < 4)  return false;
							$entry->data = \CubicleSoft\EFSS::UnpackInt(substr($data2, 0, 4));
						...
						case EFSS_DIRTYPE_FILE:
						{
							...
							if ($entry->permflags & EFSS_FLAG_INLINE)
							{
								...
								$entry->fullsize = \CubicleSoft\EFSS::UnpackInt(substr($data2, 0, 4));
								$entry->disksize = \CubicleSoft\EFSS::UnpackInt(substr($data2, 4, 2));
							...
							else
							{
								...
								$entry->fullsize = \CubicleSoft\EFSS::UnpackInt(substr($data2, 0, 8));
								$entry->disksize = \CubicleSoft\EFSS::UnpackInt(substr($data2, 8, 8));
								$entry->data = \CubicleSoft\EFSS::UnpackInt(substr($data2, 16, 4));
								if ($entry->data === false || $entry->data < 3)  return false;
							}
		...
		public function serialize()
		{
			...
			foreach ($this->entries as $entry)
			{
				...
				$data .= chr($entry->type);
				$data .= ($this->timestamp == EFSS_TIMESTAMP_UNIX ? pack("N", (int)$entry->created) : \CubicleSoft\EFSS::ConvertToUTCDateTime($entry->created));
				...
				switch ($entry->type)
				{
					...
					case EFSS_DIRTYPE_FILE:
					{
						...
						{
							$data .= \CubicleSoft\EFSS::PackInt64($entry->fullsize);
							$data .= \CubicleSoft\EFSS::PackInt64($entry->disksize);
							$data .= pack("N", $entry->data);
						}
	...
	class EFSS_Unused
	{
		...
		public function unserialize($data)
		{
			...
			for ($x = 4; $x < $y; $x += 4)  $this->unusedblocks[] = \CubicleSoft\EFSS::UnpackInt(substr($data, $x, 4));
	...
	abstract class EFSS_CopyHelper
	{
		...
		{
			if (!$this->open)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not initialized."), "errorcode" => "no_init");
		...
		public function SetStat($newstat)
		{
			...
			if ($this->realmount === true)
			{
				if (!$this->link && ($oldstat["mode"] & 07777) != ($newstat["mode"] & 07777))
				{
					...
					if (!$result)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Real 'chmod' failed on '%s'.", $this->name), "errorcode" => "setstat_chmod_error");
					$changed = true;
				}
				if (isset($newstat["uid"]) && (!isset($oldstat["uid"]) || $oldstat["uid"] != $newstat["uid"]))
				{
					...
					if (!$result)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Real '" . ($this->link ? "lchown" : "chown") . "' failed on '%s'.", $this->name), "errorcode" => ($this->link ? "setstat_lchown_error" : "setstat_chown_error"));
					$changed = true;
				}
				if (isset($newstat["gid"]) && (!isset($oldstat["gid"]) || $oldstat["gid"] != $newstat["gid"]))
				{
					...
					if (!$result)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Real '" . ($this->link ? "lchgrp" : "chgrp") . "' failed on '%s'.", $this->name), "errorcode" => ($this->link ? "setstat_lchgrp_error" : "setstat_chgrp_error"));
					$changed = true;
				}
				if (!$this->link && ($changed || $oldstat["mtime"] != $newstat["mtime"]))
				{
					$result = touch($this->name, $newstat["mtime"]);
					if (!$result)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Real 'touch' failed on '%s'.", $this->name), "errorcode" => "setstat_touch_error");
					$changed = true;
				}
			}
			else
	...
	class EFSS_DirCopyHelper extends \CubicleSoft\EFSS_CopyHelper
	{
		private $dir;
		...
		public function Init($realmount, $path, $create = false)
		{
			...
			if ($this->realmount === true)
			{
				...
				if ($this->name == "")  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to get the real path of '%s'.", $filename), "errorcode" => "unable_to_translate");

				$this->dir = opendir($this->name);
				if ($this->dir === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open '%s'.", $this->name), "errorcode" => "unable_to_open");
			}
			else
			{
		...
		public function readdir()
		{
			...
			if ($name === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Reached end of directory entries."), "errorcode" => "dir_end");
		...
		public function filetype($name)
		{
			...
			return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Does not exist."), "errorcode" => "does_not_exist");
		}
		...
		public function rmdir($pathname, $recursive = false)
		{
			...
			if (!is_dir($pathname))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Not a directory."), "errorcode" => "not_directory");
			...
			if ($dir)
			{
				while (($name = readdir($dir)) !== false)
				{
					...
					{
						if (!unlink($pathname . "/" . $name))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to unlink '%s'.", $pathname . "/" . $name), "errorcode" => "unlink_failed");
					}
				...
				if (!rmdir($pathname))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to remove directory '%s'.", $pathname), "errorcode" => "rmdir_failed");
			}
	...
	class EFSS_SymlinkCopyHelper extends \CubicleSoft\EFSS_CopyHelper
	{
		public function Init($realmount, $name, $target = false)
		{
			...
			if ($this->realmount === true)
			{
				if (!is_link($name))
				{
					if ($target === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The name '%s' is not a symlink.", $name), "errorcode" => "not_symlink");

					if (!symlink($target, $name))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to create '%s'.", $name), "errorcode" => "unable_to_create");
				}
			}
			else
			{
	...
	class EFSS_FileCopyHelper extends \CubicleSoft\EFSS_CopyHelper
	{
		private $fp;
		...
		public function Init($realmount, $filename, $mode)
		{
			...
			if ($this->realmount === true)
			{
				...
				if ($this->fp === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open '%s'.", $this->name), "errorcode" => "unable_to_open");
	...
	class EFSSIncremental
	{
		...
		public static function GetLock($lockfile, $writelock, $waitforlock = true, $maxtime = 20)
		{
			...
			$readwritelock = new \CubicleSoft\ReadWriteLock($lockfile);
			if (!$readwritelock->Lock($writelock, $waitforlock))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to obtain lock."), "errorcode" => "lock_failed");
		...
		{
			if (!file_exists($filename) || !file_exists($filename . ".updates") || !file_exists($filename . ".serial") || file_exists($filename . ".blocknums") || file_exists($filename . ".partial"))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Encrypted file storage system does not exist."), "errorcode" => "does_not_exist");
			if ($blocksize == 0 || $blocksize % 4096 != 0 || $blocksize > 32768)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The block size must be a multiple of 4096 and able to fit into an 'unsigned short'."), "errorcode" => "invalid_blocksize");
			...
			if (is_string($since))
			{
				...
				else  $since = \CubicleSoft\EFSS::ConvertFromUTCDateTime($since);
			}

			$fp = fopen($filename, "rb");
			if ($fp === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open block file."), "errorcode" => "block_file_open");
			$fp2 = fopen($filename . ".updates", "rb");
			if ($fp2 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open updates file."), "errorcode" => "updates_file_open");
			if (!file_exists($filename . ".hashes"))  $fp3 = false;
			else
			{
				$fp3 = fopen($filename . ".hashes", "rb");
				if ($fp3 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open hashes file."), "errorcode" => "hashes_file_open");
			}

			// Read in the last written timestamp and compare it to any original.
			$lastwrite = \CubicleSoft\EFSS::ConvertFromUTCDateTime(fread($fp2, 20));
			if ($startblock > 0 && $lastwrite !== $origlastwrite)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Write between reads detected."), "errorcode" => "write_between_detected");

			// Move to the starting point for the data.
			\CubicleSoft\EFSS::RawSeek($fp, $blocksize * $startblock);
			\CubicleSoft\EFSS::RawSeek($fp2, 20 * $startblock);
			if ($fp3 !== false)  \CubicleSoft\EFSS::RawSeek($fp3, 36 * $startblock);
			...
			if ($readblocks == 0)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Invalid length specified."), "errorcode" => "invalid_length");
			$numblocks = 0;
			do
			{
				...
				{
					if (\CubicleSoft\EFSS::ConvertFromUTCDateTime(substr($data2, $x * 20, 20)) > $since)
					{
						if ($fp3 !== false)
						{
							$data4 = substr($data, $x * $blocksize, $blocksize) . substr($data2, $x * 20, 20);
							if (substr($data3, $x * 36, 16) !== md5($data4, true) || substr($data3, ($x * 36) + 16, 20) !== sha1($data4, true))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Hash failure detected.  Possible data corruption in the original backup file."), "errorcode" => "invalid_hash");
						}
		...
		public static function Write($filename, $startblock, $blockdata, $lastupdateddata, $md5, $sha1, $blocknumdata = false, $serial = false, $blocksize = 4096, $finalize = false)
		{
			...
			if ($md5 !== md5($data) || $sha1 !== sha1($data))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("One of the hashes does not match."), "errorcode" => "invalid_hash");
			unset($data);

			$mode = "wb";
			if ($startblock < 0)  $startblock = 0;
			if ($startblock == 0)  \CubicleSoft\EFSSIncremental::Delete($filename);
			else if (!file_exists($filename) || !file_exists($filename . ".updates") || !file_exists($filename . ".hashes") || !file_exists($filename . ".serial") || ($blocknumdata !== false && !file_exists($filename . ".blocknums")) || !file_exists($filename . ".partial"))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Encrypted file storage system incremental '%s' does not exist.", $filename), "errorcode" => "does_not_exist");
			else  $mode = "r+b";
			...
			if ($fp === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open updates file."), "errorcode" => "updates_file_open");
			\CubicleSoft\EFSS::RawSeek($fp, 20 * $startblock);
			...
			if ($fp === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open hashes file."), "errorcode" => "hashes_file_open");
			\CubicleSoft\EFSS::RawSeek($fp, 36 * $startblock);
			fwrite($fp, $hashes);
			fclose($fp);
			...
			if ($blocknumdata !== false)
			{
				...
				if ($fp === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open block numbers file."), "errorcode" => "block_nums_file_open");
				\CubicleSoft\EFSS::RawSeek($fp, 4 * $startblock);
		...
		{
			if (!file_exists($filename) || !file_exists($filename . ".updates") || !file_exists($filename . ".hashes") || !file_exists($filename . ".serial"))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Encrypted file storage system does not exist."), "errorcode" => "does_not_exist");
		...
		{
			if (!file_exists($filename . ".updates"))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Encrypted file storage system or incremental update file does not exist."), "errorcode" => "does_not_exist");
			...
			if ($result === false || strlen($result) < 20)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to determine last update."), "errorcode" => "updates_file_invalid");
		...
		{
			if (!file_exists($basefile) || !file_exists($basefile . ".updates") || !file_exists($basefile . ".hashes") || !file_exists($basefile . ".serial") || file_exists($basefile . ".blocknums") || file_exists($basefile . ".partial"))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Encrypted file storage system does not exist."), "errorcode" => "does_not_exist");
			if (!file_exists($incrementalfile) || !file_exists($incrementalfile . ".updates") || !file_exists($incrementalfile . ".hashes") || !file_exists($incrementalfile . ".serial") || !file_exists($incrementalfile . ".blocknums") || file_exists($incrementalfile . ".partial"))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Encrypted file storage system incremental '%s' does not exist.", $incrementalfile), "errorcode" => "does_not_exist");
			if ($blocksize == 0 || $blocksize % 4096 != 0 || $blocksize > 32768)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("The block size must be a multiple of 4096 and able to fit into an 'unsigned short'."), "errorcode" => "invalid_blocksize");

			if (trim(file_get_contents($basefile . ".serial")) !== trim(file_get_contents($incrementalfile . ".serial")))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Incremental block file '%s' serial does not match base file serial.", $incrementalfile), "errorcode" => "increment_serial_mismatch");

			// Open file handles.
			$basefp = fopen($basefile, "r+b");
			if ($basefp === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open block file."), "errorcode" => "block_file_open");
			$basefp2 = fopen($basefile . ".updates", "r+b");
			if ($basefp2 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open updates file."), "errorcode" => "updates_file_open");
			$basefp3 = fopen($basefile . ".hashes", "r+b");
			if ($basefp3 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open hashes file."), "errorcode" => "hashes_file_open");

			$incfp = fopen($incrementalfile, "rb");
			if ($incfp === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open incremental block file."), "errorcode" => "block_file_open");
			$incfp2 = fopen($incrementalfile . ".updates", "rb");
			if ($incfp2 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open incremental updates file."), "errorcode" => "updates_file_open");
			$incfp3 = fopen($incrementalfile . ".hashes", "rb");
			if ($incfp3 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open incremental hashes file."), "errorcode" => "hashes_file_open");
			$incfp4 = fopen($incrementalfile . ".blocknums", "rb");
			if ($incfp4 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open incremental block numbers file."), "errorcode" => "block_nums_file_open");
			...
			while ($incblockdata !== false && $incblockdata != "")
			{
				...
				for ($x = 0; $x < $y; $x++)
				{
					$blocknum = \CubicleSoft\EFSS::UnpackInt(substr($incblocknums, $x * 4, 4));

					\CubicleSoft\EFSS::RawSeek($basefp, $blocksize * $blocknum);
					fwrite($basefp, substr($incblockdata, $x * $blocksize, $blocksize));

					\CubicleSoft\EFSS::RawSeek($basefp2, 20 * $blocknum);
					fwrite($basefp2, substr($incupdates, $x * 20, 20));

					\CubicleSoft\EFSS::RawSeek($basefp3, 36 * $blocknum);
			...
			if ($delete)
			{
				\CubicleSoft\EFSSIncremental::Delete($incrementalfile);
		...
		{
			if (!file_exists($incrementalfile) || !file_exists($incrementalfile . ".updates") || !file_exists($incrementalfile . ".serial") || file_exists($incrementalfile . ".partial"))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Encrypted file storage system incremental does not exist."), "errorcode" => "does_not_exist");
			...
			$startts = microtime(true);
			$blocksprocessed = 0;
			$totalblocks = (int)(\CubicleSoft\EFSS::RawFileSize($incfp) / $blocksize);
			fseek($incfp, 0, SEEK_SET);

			if ($key1 == "" || $iv1 == "" || $key2 == "" || $iv2 == "")
			{
				if (!file_exists($incrementalfile . ".hashes"))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Encrypted file storage system incremental hashes file does not exist."), "errorcode" => "does_not_exist");
				...
				while ($incblockdata !== false && $incblockdata != "")
				{
					...
					for ($x = 0; $x < $y; $x++)
					{
						...
						if (substr($inchashes, $x * 36, 16) !== md5($data, true) || substr($inchashes, ($x * 36) + 16, 20) !== sha1($data, true))  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Hash failure detected.  Possible data corruption."), "errorcode" => "invalid_hash");
					}
			...
			else
			{
				...
				while ($incblockdata !== false && $incblockdata != "")
				{
					...
					for ($x = 0; $x < $y; $x += $blocksize)
					{
						...
						$result = \CubicleSoft\EFSS::RawDecryptBlock($cipher1, $cipher2, $block, EFSS_BLOCKTYPE_ANY);
						if (!$result["success"])  return $result;

						if ($firstblock && $result["type"] !== EFSS_BLOCKTYPE_FIRST)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("First block is not of the correct type."), "errorcode" => "block_type");
						else if (!$firstblock && $result["type"] !== EFSS_BLOCKTYPE_DIR && $result["type"] !== EFSS_BLOCKTYPE_FILE && $result["type"] !== EFSS_BLOCKTYPE_UNUSED_LIST && $result["type"] !== EFSS_BLOCKTYPE_UNUSED)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("A block in the file does not have a valid type."), "errorcode" => "block_type");
		...
		public static function MakeReverseDiffIncremental($basefile, $incrementalfile, $blocksize = 4096)
		{
			...
			if ($basefp === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open reverse diff block file."), "errorcode" => "rdiff_block_file_open");
			$basefp2 = fopen($basefile . ".rdiff.updates", "r+b");
			if ($basefp2 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open reverse diff updates file."), "errorcode" => "rdiff_updates_file_open");
			$basefp3 = fopen($basefile . ".rdiff.hashes", "r+b");
			if ($basefp3 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open reverse diff hashes file."), "errorcode" => "rdiff_hashes_file_open");
			$basefp4 = fopen($basefile . ".rdiff.blockmap", "r+b");
			if ($basefp4 === false)  return array("success" => false, "error" => \CubicleSoft\EFSS::Translate("Unable to open reverse diff block mapping file."), "errorcode" => "rdiff_block_map_file_open");
			...
			while ($diffmap !== false && $diffmap != "")
			{
				...
				for ($x = 0; $x + 3 < $y; $x += 4)
				{
					...
					if ($data !== "\xFF\xFF\xFF\xFF")
					{
						$pos = \CubicleSoft\EFSS::UnpackInt($data);

						\CubicleSoft\EFSS::RawSeek($basefp, $blocksize * $pos);
						fwrite($incfp, fread($basefp, $blocksize));

						\CubicleSoft\EFSS::RawSeek($basefp2, 20 * $pos);
						fwrite($incfp2, fread($basefp2, 20));

						\CubicleSoft\EFSS::RawSeek($basefp3, 36 * $pos);
