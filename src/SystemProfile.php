...
?><?php
	...
	class SystemProfile
	{
		...
		public static function GetProfile()
		{
			...
			$result = array(
				"hostname" => \CubicleSoft\SystemProfile::GetHostname(),
				"machine_id" => \CubicleSoft\SystemProfile::GetMachineID(),
				"motherboard" => \CubicleSoft\SystemProfile::GetMotherboardInfo(),
				"cpu" => \CubicleSoft\SystemProfile::GetCPUInfo(),
				"ram" => \CubicleSoft\SystemProfile::GetRAMInfo(),
				"gpu" => \CubicleSoft\SystemProfile::GetGPUInfo(),
				"nic" => \CubicleSoft\SystemProfile::GetNICInfo(),
				"disk" => \CubicleSoft\SystemProfile::GetDiskInfo(),
				"os" => \CubicleSoft\SystemProfile::GetOSInfo(),
			);
		...
		protected static function RunCommand($cmd)
		{
			...
			if ($windows && class_exists("\\CubicleSoft\\ProcessHelper", false))
			{
				$options = array(
					"stdin" => false,
					"stdout" => true,
				...
				$result = \CubicleSoft\ProcessHelper::StartProcess($cmd, $options);
				if ($result["success"])
				{
					$result2 = \CubicleSoft\ProcessHelper::Wait($result["proc"], $result["pipes"]);
