...
?><?php
	...
	class IFDS
	{
		...
		public function Create($pfcfilename, $appmajorver, $appminorver, $appbuildnum, $magic = false, $ifdsfeatures = self::FEATURES_OBJECT_ID_STRUCT_SIZE, $fmtfeatures = 0)
		{
			...
			if (class_exists("\\CubicleSoft\\PagingFileCache", false) && $pfcfilename instanceof PagingFileCache)
			{
				if ($pfcfilename->GetMaxPos() > 0)  return array("success" => false, "error" => self::IFDSTranslate("The file already exists."), "errorcode" => "file_exists", "info" => "\\CubicleSoft\\PagingFileCache");
		...
		public function Open($pfcfilename, $magic = false)
		{
			...
			if (class_exists("\\CubicleSoft\\PagingFileCache", false) && $pfcfilename instanceof PagingFileCache)
			{
				...
				if ($this->maxpos === 0)  return array("success" => false, "error" => self::IFDSTranslate("No file data.  For streaming content, use stream reader."), "errorcode" => "no_file_data", "info" => "\\CubicleSoft\\PagingFileCache");
...
//var_dump($this->idmap);
		...
		public static function OptimizeCopyObjectInternal($srcifds, $destifds, $obj)
		{
			...
			$obj2 = new \CubicleSoft\IFDS_RefCountObj($data);
		...
		public function CreateObject($type, $dataencodernum, $name, $typeinfo, $extra, $withid = true)
		{
			...
			return array("success" => true, "obj" => new \CubicleSoft\IFDS_RefCountObj($data));
		}
		...
		{
			return (($obj->data["enc"] & \CubicleSoft\IFDS::ENCODER_MASK_DATA) === \CubicleSoft\IFDS::ENCODER_DATA_CHUNKS_STREAM);
		}
		...
		public function ExtractNextStructure(&$data, &$pos, $size, $raw = false)
		{
			...
			{
				$obj = new \CubicleSoft\IFDS_RefCountObj($this->objcache[$id]);
		...
		public function GetObjectByPosition($filepos, $size = 4092)
		{
			...
			return array("success" => true, "obj" => new \CubicleSoft\IFDS_RefCountObj($this->objcache[$this->objposmap[$filepos]]));
		}
		...
		public function GetObjectByID($id, $updatelastaccess = true)
		{
			...
			return array("success" => true, "obj" => new \CubicleSoft\IFDS_RefCountObj($this->objcache[$id]));
		}
...
//var_dump($this->freemap);
	...
	class IFDS_RefCountObj
	{
		...
		{
			return ($this->data["type"] & \CubicleSoft\IFDS::TYPE_BASE_MASK);
		}
		...
		{
			return ($this->data["enc"] & \CubicleSoft\IFDS::ENCODER_MASK_DATA_NUM);
		}

		public function GetDataMethod()
		{
			return ($this->data["enc"] & \CubicleSoft\IFDS::ENCODER_MASK_DATA);
		}
		...
		{
			return ($this->data["enc"] === (\CubicleSoft\IFDS::ENCODER_NONE | \CubicleSoft\IFDS::ENCODER_NO_DATA));
		}
		...
		{
			return (($this->data["enc"] & \CubicleSoft\IFDS::ENCODER_MASK_DATA) === \CubicleSoft\IFDS::ENCODER_DATA_CHUNKS_STREAM);
		}
